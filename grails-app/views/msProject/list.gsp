	...
	<head>
		...
		<script>
			function deleteFormatter(value, row) {
				var str='<a href="${request.contextPath}/msProject/delete/'+row.id+'"><button class="btn btn-default margin" type="button"><span class="glyphicon glyphicon-trash"></span> &nbsp;<g:message code="default.button.delete.label" default="Delete" /></button></a>';
				return str;
			}
			function editFormatter(value, row) {
				var str='<a href="${request.contextPath}/msProject/edit/'+row.id+'"><button class="btn btn-default margin" type="button"><span class="glyphicon glyphicon-edit"></span> &nbsp;<g:message code="default.button.edit.label" default="Edit" /></button></a>';
				return str;
			}
			function nameFormatter(value, row) {
				var str='<a href="${request.contextPath}/msProject/show/'+row.id+'">'+row.name+'</a>';
				return str;
			}
			...
			function deleteAll(){
				var selects=$('#msProjectTable').bootstrapTable('getSelections');
				if(selects.length>0){
					var ids=new Array();
					...
					if(confirm('${message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}')) {
						$.post("${request.contextPath}/msProject/deleteAll", obj,
								function (data, textStatus) {
									if (data.result) {
										...
										$('#msProjectTable').bootstrapTable('refresh',[]);
									} else {
			...
			function importExcel(){
				$('#excelForm').form('submit', {
					url:'${request.contextPath}/msProject/importExel',
			...
			function newWindow(){
				document.location.href="${createLink(controller: 'msProject',action:'create')}";
			}
		</script>
	</head>
	<body>
	...
	<div >
		...
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active" ><g:message code="msProject.label" default="MsProject" /></li>
			</ol>
		</div><!--/.row-->
		...
		<!--
			<div class="col-lg-12">
				<h1 class="page-header"><g:message code="msProject.label" default="MsProject" /></h1>
			</div>
			-->
				...
				<div class="panel panel-default">
					<g:form method="post">
						...
						<div class="panel-body">
							...
							<table id="msProjectTable" data-toggle="table"  data-toolbar="#toolbar"
								   data-url="${request.contextPath}/msProject/json"   data-cache="false"
								   ...
								   data-select-item-name="checkIds" data-sort-name="projectName" data-sort-order="desc">
								<thead>
								<tr>
									<th data-field="nofield" data-checkbox="true"></th>
									<th data-field="id"  data-sortable="true" >id</th>
									
									<th data-field="projectName"  data-sortable="true"   >${message(code: 'msProject.projectName.label', default: 'Project Name')}</th>
									
									<th data-field="rigName"  data-sortable="true"   >${message(code: 'msProject.rigName.label', default: 'Rig Name')}</th>
									


									<th data-field="msUser.text"  >${message(code: 'msProject.msUser.label', default: 'Ms User')}</th>

									
									<th data-field="remark"  data-sortable="true"   >${message(code: 'msProject.remark.label', default: 'Remark')}</th>
									
									<th data-field="dateCreated"  data-sortable="true"   >${message(code: 'msProject.dateCreated.label', default: 'Date Created')}</th>
									
									<th data-field="lastUpdated"  data-sortable="true"   >${message(code: 'msProject.lastUpdated.label', default: 'Last Updated')}</th>
